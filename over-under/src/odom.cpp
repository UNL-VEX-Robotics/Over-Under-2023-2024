...
int odomTask() {
	...
	if(arms::selector::auton == 1 || arms::selector::auton == 3){
		x_offset = -59;
		y_offset = 33;
		angle_offset = 180;
	}
	if(arms::selector::auton == 2 || arms::selector::auton == 4){
		x_offset = 12;
		y_offset = -55.75;
		angle_offset = 90;
	}
	...
	prev_heading = (-imu->get_rotation() * M_PI / 180.0) + (angle_offset * M_PI / 180.0);

	if(prev_heading - angle_offset != 0){
		printf("Calibrating... \n");
		imu->reset(true);
	...
	while (true) {
		...
		double delta_angle;
		if (imu) {
			heading = (-imu->get_rotation() * M_PI / 180.0) + (angle_offset * M_PI / 180.0);
