...
PID::PID(double P_weight, double I_weight, double D_weight, double toleranceMargin, double integralMaxError, double limit){
    this->error = 0;
...
double PID::getNextValue(double currentError){
    ...
    return fmin(abs(output), outputLimit) * sign(output); 
}
